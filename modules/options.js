...
export class StreamViewOptions {
	...
	static init() {
		...
		// V13: AVSettings.DOCK_POSITIONS may not exist; provide safe fallback
		const dockPositions = AVSettings?.DOCK_POSITIONS ?? { LEFT: 'left', RIGHT: 'right', TOP: 'top', BOTTOM: 'bottom' };
		...
		game.settings.register('stream-view', 'voice-video-position', {
			...
			restricted: true,
			requiresReload: true,
			choices: Object.fromEntries(Object.values(dockPositions).map(p => {
				const key = p.charAt(0).toUpperCase() + p.slice(1);
				return [p, game.i18n.localize(`WEBRTC.DockPosition${key}`) || p];
			})),
			default: dockPositions.LEFT,
			type: String,
		});
		...
		// V13: AVSettings.NAMEPLATE_MODES may not exist; provide safe fallback
		const nameplateModes = AVSettings?.NAMEPLATE_MODES ?? { OFF: 0, PLAYER_ONLY: 1, CHAR_ONLY: 2, BOTH: 3 };
		...
		game.settings.register('stream-view', 'voice-video-nameplate-mode', {
			...
			requiresReload: true,
			choices: {
				[nameplateModes.OFF]: game.i18n.localize('WEBRTC.NameplatesOff'),
				[nameplateModes.PLAYER_ONLY]: game.i18n.localize('WEBRTC.NameplatesPlayer'),
				[nameplateModes.CHAR_ONLY]: game.i18n.localize('WEBRTC.NameplatesCharacter'),
				[nameplateModes.BOTH]: game.i18n.localize('WEBRTC.NameplatesBoth'),
			},
			default: nameplateModes.BOTH,
			type: Number,
		});
	...
	static #defaultPadding(inCombat = false) {
		...
		// V13: SidebarTab.defaultOptions may not exist; use safe fallback
		const sidebarWidth = SidebarTab?.defaultOptions?.width ?? 300;

		const dockPositions = AVSettings?.DOCK_POSITIONS ?? { LEFT: 'left', RIGHT: 'right', TOP: 'top', BOTTOM: 'bottom' };
		const rtcSettings = game.settings.get('core', 'rtcWorldSettings');
		if (
			rtcSettings.mode !== (AVSettings?.AV_MODES?.DISABLED ?? 'disabled') &&
