...
export class StreamViewGM extends StreamView {
	...
	setup() {
		...
		// V13: getSceneControlButtons signature changed — now receives (controls, groups) or just (controls)
		Hooks.on('getSceneControlButtons', (controls) => this.#addStreamControls(controls));
		...
		Hooks.on('renderTokenHUD', (_app, html, tokenHUD) => this.#handleTokenHUD(html instanceof jQuery ? html : $(html), tokenHUD));
	...
	/**
	 * V13: getSceneControlButtons passes an array of control objects.
	 * The structure is the same as before, so this method is unchanged,
	 * but we add a guard in case `controls` is not a plain array.
	 *
	 * @param {SceneControl[]|Object} controls
	 * @private
	 */
	#addStreamControls(controls) {
		// In V13, controls is still an array
	...
	#handleTokenHUD(html, tokenHUD) {
		...
		// V13: tokenHUD may have .object instead of ._id
		const tokenId = tokenHUD._id ?? tokenHUD.object?.id;
