...
// Helper: normalize html arg — V13 AppV2 passes HTMLElement, V11/12 passes jQuery
function jq(html) {
	if (html instanceof jQuery) return html;
	return $(html);
}

// Math.clamped was removed in V13; use Math.clamp (V13+) with fallback
function clamp(value, min, max) {
	if (typeof Math.clamp === 'function') return Math.clamp(value, min, max);
	return Math.clamped(value, min, max);
}
...
export class StreamViewStream extends StreamView {
	...
	setup() {
		...
		Hooks.on('renderSceneControls', (_app, html) => this.#hideHtml(jq(html)));
		Hooks.on('renderHotbar', (_app, html) => this.#hideHtml(jq(html)));
		Hooks.on('renderSidebar', (_app, html) => {
			if (!game.settings.get('stream-view', 'show-full-sidebar')) {
				this.#hideHtml(jq(html))
			}
		});
		Hooks.on('renderHeadsUpDisplay', (_app, html) => this.#appendSpeechBubblesContainer(jq(html)));
		Hooks.on('renderSidebarTab', (app, html) => this.#handlePopout(app, jq(html)));
		Hooks.on('renderUserConfig', (app, html) => this.#handlePopout(app, jq(html)));
		...
		Hooks.on('renderApplication', (app, html) => this.#handlePopout(app, jq(html)));
		Hooks.on('renderItemSheet', (app, html) => this.#handlePopout(app, jq(html)));
		Hooks.on('renderActorSheet', (app, html) => this.#handlePopout(app, jq(html)));
		Hooks.on('renderCameraViews', (_app, html) => this.#handleRenderCameraViews(jq(html)));
	...
	#ready() {
		...
		if (!game.settings.get('stream-view', 'show-logo')) {
			// V13: logo element selector may have changed
			$('img#logo, #logo').hide();
		}
		...
		// V13: game.webrtc may not exist if AV is disabled
		if (game.webrtc) {
		...
		// V13: CameraViews.prototype.setUserIsSpeaking may have been removed/renamed.
		// Try to register — libWrapper will throw if the method doesn't exist.
		try {
			libWrapper.register(
				'stream-view',
			...
			console.warn('StreamView: Could not wrap CameraViews.prototype.setUserIsSpeaking — voice speaking detection may be unavailable.', e);
		}

		// V13: SoundsLayer may be renamed or removed. Try to suppress it gracefully.
		try {
			libWrapper.register(
				'stream-view',
			...
			console.warn('StreamView: Could not wrap SoundsLayer.prototype.refresh — ambient sounds may play for stream user.', e);
		}

		// V13: ChatLog popout / scrollBottom behavior may have changed
		try {
			libWrapper.register(
				'stream-view',
			...
			console.warn('StreamView: Could not wrap ChatLog.prototype.scrollBottom.', e);
		}
		...
		// V13: ui.sidebar.tabs may have changed structure
		if (game.settings.get('stream-view', 'show-chat') && !game.settings.get('stream-view', 'show-full-sidebar')) {
			const chatTab = ui.sidebar?.tabs?.chat ?? ui.chat;
			if (chatTab) {
				this.#createPopout(StreamViewOptions.PopoutIdentifiers.CHAT, chatTab);
			}
		}
	...
	#coordBounds(coords = []) {
		...
		// V13: canvas.grid.w/h may be canvas.grid.sizeX/sizeY
		const gridW = canvas.grid.w ?? canvas.grid.sizeX ?? canvas.grid.size ?? 100;
		const gridH = canvas.grid.h ?? canvas.grid.sizeY ?? canvas.grid.size ?? 100;
		const gridPadding = {
			w: gridW * 1.5,
			h: gridH * 1.5,
		};
		...
		const scale = Math.round(clamp(ratio, minScale, maxScale) * 100) / 100;
	...
	async #animateTo({ x, y, scale }) {
		...
		// V13: canvas.getLayerByEmbeddedName may not find AmbientSound; guard it
		try {
	...
	#getInitialViewport() {
		...
		x ??= (r.right / 2);
		y ??= (r.bottom / 2);
		scale ??= clamp(Math.min(window.innerHeight / r.height, window.innerWidth / r.width), 0.25, 3);
		return { x, y, scale };
	}
	...
	#handleRenderCameraViews(html) {
		...
		const dockPositions = AVSettings?.DOCK_POSITIONS ?? { TOP: 'top', BOTTOM: 'bottom' };
		...
		const isVertical = [dockPositions.TOP, dockPositions.BOTTOM].includes(position);
	...
	#createPopout(identifier, app) {
		...
		// V13: createPopout may not exist on AppV2; guard it
		if (typeof app.createPopout !== 'function') {
			console.warn(`StreamView: ${identifier} tab does not support createPopout() in this version of Foundry.`);
			return;
		}
	...
	#updateCombat(active, combat) {
		...
		if (game.settings.get('stream-view', 'auto-show-combat')) {
			...
			// V13: ui.sidebar.tabs may have changed
			const combatTab = ui.sidebar?.tabs?.combat ?? ui.combat;
			if (combatTab) {
				this.#createPopout(StreamViewOptions.PopoutIdentifiers.COMBAT, combatTab);
			}
		}
	...
	#tokenCoords(tokens) {
		...
		tokens.forEach((t) => {
			...
			coords.push({ x: t.document.x, y: t.document.y + t.h });
		});
		return coords;
	}
	...
	async #tokenForGM(userId) {
		...
		let tokenId;
		try {
			const tokenIDs = await this._socket.executeAsUser('controlledTokens', userId);
	...
	#hidePopoutHeaders(html) {
		...
		// V13: header may use different selector
		html.children('header.window-header, header.application-header').hide();
	}
	...
	#toggleNotes(toggled) {
		...
		// V13: NotesLayer may have changed
		const notesLayerName = NotesLayer?.layerOptions?.name ?? 'notes';
		const toggleSetting = NotesLayer?.TOGGLE_SETTING ?? 'notesDisplayToggle';
		canvas[notesLayerName]?.activate();
		game.settings.set("core", toggleSetting, toggled);
		if (currentLayer) canvas[currentLayer]?.activate();
	...
	#getNotesStatus() {
		const toggleSetting = NotesLayer?.TOGGLE_SETTING ?? 'notesDisplayToggle';
		return game.settings.get("core", toggleSetting);
	}
}
